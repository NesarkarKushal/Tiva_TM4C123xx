...
void SPI_Init()
{
   ...
   //    SSIClockSourceSet(SSI3_BASE,SSI_CLOCK_SYSTEM);
       SSIConfigSetExpClk(SSI3_BASE,SysCtlClockGet(),SSI_FRF_MOTO_MODE_3,SSI_MODE_MASTER,4000000,8);
      // SSILoopbackEnable(SSI3_BASE);
       SSIEnable(SSI3_BASE);
//       IntEnable(INT_SSI0);
...
//   GPIOPinWrite(GPIO_PORTC_BASE,0X40,0X40);// DC PIN =PC6
  ...
  {
    SSIDataPut(SSI3_BASE,ASCII[character - 0x20][index]);
  }
...
{
    int row[5]={0x40,0x41,0x42,0x43,0x44,0x45};

   GPIOPinWrite(GPIO_PORTC_BASE,0X40,0X00);
   SSIDataPut(SSI3_BASE,0X20);//Horizontal display V=0
   SysCtlDelay(13333333);
   SSIDataPut(SSI3_BASE,row[y]);  // Row.
   SSIDataPut(SSI3_BASE, (0x80 | x));  // Column.
   SysCtlDelay(13333333);
...
void Reset_Init()
{
    // GPIO , RESET and Chip select and D/C
